...
private:
  ...
  ros::Publisher  pubError_; // Task error
...
VS::VS(int argc, char**argv)
{
  ...
  pubError_ = nh_.advertise<std_msgs::Float32MultiArray>("/TaskError", 1000);
...
void VS::init_vs()
{
  ...
  lambda_adapt.initStandard(1.0, 0.6, 10);
  ...
  eJe.resize(6, 3); // pioneer pepper expressed at frame base
  eJe = 0;
  eJe[0][0] = 1; // vx
  eJe[1][1] = 1; // vy
  eJe[5][2] = 1; // wz

  vpTranslationVector e_t_c(0.03180346870612999, -0.004473436888500982, 1.166312795678887);
  vpQuaternionVector e_q_c(-0.4531162252060521, 0.4582191211825679, -0.5386802386053814, 0.542719562917851);

  vpHomogeneousMatrix eVc(e_t_c,e_q_c);
...
void VS::poseCallback(const geometry_msgs::PoseStampedConstPtr& msg)
{
  ...
  try {
    ...
    out_cmd_vel.linear.y = v[1];
    out_cmd_vel.linear.z = 0;
    out_cmd_vel.angular.x = 0;
    out_cmd_vel.angular.y = 0;
    out_cmd_vel.angular.z = v[2];
